...
struct ScheduleView: View {
    @State private var selectedDay = 2 // Wednesday = index 2
        ...
        ZStack(alignment: .bottomTrailing) {
            ...
            Button(action: {}) {
                Image(systemName: "plus")
                    ...
                    .clipShape(Circle())
                    .shadow(color: Color(hex: "F5C518").opacity(0.5), radius: 10, x: 0, y: 5)
            }
            .padding(.trailing, 20)
            .padding(.bottom, 90)
        }
    ...
    var timelineView: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack {
                    Spacer().frame(width: 60)
                    WalkIndicator(text: "8 min walk to next class")
                    Spacer()
                }
                Spacer().frame(height: 8)

                // 10:00 slot — HCI (live, yellow)
                HStack(alignment: .top, spacing: 0) {
                ...
                HStack {
                    Spacer().frame(width: 60)
                    LunchIndicator(text: "Lunch Break • 45 min")
                    Spacer()
                }
                Spacer().frame(height: 8)

                // 13:00 slot — Digital Marketing
                HStack(alignment: .top, spacing: 0) {
                    Spacer().frame(width: 60)
